#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 意优RP关节 测试 PLAN（node 指定要测试的电机节点ID）。

用法:
    python3 tools/gen_plan_yrp.py 1                 # 测试 node=1
    python3 tools/gen_plan_yrp.py 3                 # 测试 node=3
    ...
    cases += [
        ...
        case("⑤ 往返±30(60°)", "measurement", "measure_yrp_roundtrip_all",
             params=[param("node", "int", node),
                     ...
                     param("velocity_deg_s", "float", 10), param("acc_deg_s2", "float", 30)],
             returns=[{"item": "value", "type": "float", "judge": "范围内", "threshold": "58~62"}],
             timeout=80000),
        delay(1000),
        case("⑥ 回0位", "measurement", "measure_yrp_home0_all",
             params=[param("node", "int", node), param("tolerance", "float", 1.0)],
             returns=[{"item": "value", "type": "float", "judge": "范围内", "threshold": "-1~1"}],
             timeout=60000),
        delay(1000),
        case("⑦ 角度/编码器回读", "measurement", "measure_yrp_read_all",
             params=[param("node", "int", node)],
             returns=[{"item": "value", "type": "float", "judge": "", "threshold": ""}],
             timeout=15000),
        delay(1000),
        case("⑧ 失能", "action", "action_yrp_disable_all",
             params=[param("node", "int", node)], timeout=8000),
        delay(1000),
        case("⑨ 断开CAN", "action", "action_yrp_disconnect", timeout=8000),
    ]
    return {
        "name": "意优RP关节CANopen测试",
        "description": "单序列: 连接→读设备信息→使能标0→±60/±30往返→回0→回读→失能断开, 步骤间延时1s。node参数指定测试节点(node%d)。" % node,
        "variables": {},
        "settings": {
...
def main():
    node = parse_node(sys.argv[1] if len(sys.argv) > 1 else "")
    plan = build(node)
    ...
    print("已生成: %s\n节点: node%d | 序列: %d | 用例: %d" % (PLAN_OUT, node, len(plan["sequences"]), n))
